...
async def fetch_youtube_link(query):
    try:
        url = f"https://steep-hall-e08e.kustbotsweb.workers.dev/search?q={query}"
...
async def start_handler(_, message):
    ...
    buttons = [
        [
            ...
            InlineKeyboardButton(f"📢 {updates_text}", url="https://t.me/ArthurXMafia")
        ],
        [
            InlineKeyboardButton(f"💬 {support_text}", url="https://t.me/chatsandchill"),
    ...
    await message.reply_animation(
        animation="https://frozen-imageapi.lagendplayersyt.workers.dev/file/713e5784-b8f7-4f1d-88fd-6f47984e01b5.mp4",
        caption=caption,
...
async def process_play_command(message: Message, query: str):
    ...
    if isinstance(result, dict) and "playlist" in result:
        ...
        if secs > MAX_DURATION_SECONDS:
            await processing_message.edit(
                "❌ Streams longer than 2 hours are not allowed."
            )
            return
...
async def fallback_local_playback(chat_id: int, message: Message, song_info: dict):
    ...
    try:
        ...
        base_caption = (
            "<blockquote>"
            "<b> 𝚨𝐋𝚬𝐗𝚨 𝐗 𝚳𝐔𝐒𝚰𝐂 Streaming</b> (Local Playback)\n\n"
